...
import tbhizzle.rt6.cowkiller.CowKiller;
import tbhizzle.rt6.cowkiller.data.*;
import tbhizzle.rt6.cowkiller.tasks.TanHides;

public class CowKilerGui extends JFrame {
	CowKiller cowKiller;
	...
	//final JComboBox<MyItem> comboBoxCrafter;
	final JComboBox comboBoxCrafter;
...
//		});
//	}
	public CowKilerGui(CowKiller c){
		cowKiller = c;
		...
		//comboBoxCrafter = new JComboBox<MyItem>();
		comboBoxCrafter = new JComboBox();
		...
		rdbtnLeather.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				cowKiller.setTanningMyItem(TanHides.LEATHER);
		...
		rdbtnHardLeather.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				cowKiller.setTanningMyItem(TanHides.HARD_LEATHER);
		...
		comboBoxCrafter.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				MyItem myItem = (MyItem)comboBoxCrafter.getSelectedItem();
				cowKiller.setCraftingMyItem(myItem);
			}
		});
	...
	private void setCraftingItems(boolean softLeather){
		...
		if(softLeather){
			...
			for(LeatherItem l: LeatherItem.values()){
				comboBoxCrafter.addItem(l.getMyItem());
			}
		}else{
